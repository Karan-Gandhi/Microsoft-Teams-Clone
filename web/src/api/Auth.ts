import { LoginRequest, RenewAccessTokenRequest } from "../types/Requests";
import { LoginResponse, RenewAccessTokenResponse } from "../types/Responses";
...
export const loginWithEmailAndPassword = async (email: string, password: string) => {
	...
	try {
		const data = await fetchUsingPOST<LoginRequest, LoginResponse>(APIRoutes.LOGIN, { email, password });
...
export const renewAccessToken = async (): Promise<AccessToken> => {
	...
	try {
		const data = await fetchUsingPOST<RenewAccessTokenRequest, RenewAccessTokenResponse>(APIRoutes.RENEW_ACCESS_TOKEN, { refreshToken });
		setCookie(CookieNames.ACCESS_TOKEN_COOKIE_NAME, data.accessToken as string);
		setCookie(CookieNames.REFRESH_TOKEN_COOKIE_NAME, data.refreshToken as string);
		setCookie(CookieNames.ACCESS_TOKEN_TYPE_COOKIE_NAME, data.type);
